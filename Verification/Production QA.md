...
3. **Precondition gating (next).** Promote the soft warnings (not on AC, hot ambient, busy system, chassis not soaked) into hard preconditions that abort or quarantine a run, so every scored run was taken under comparable conditions. Without this, the baseline comparison is apples-to-oranges across rooms.
